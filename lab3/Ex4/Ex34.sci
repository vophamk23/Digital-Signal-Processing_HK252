// ============================================================
// FUNCTION : add
// PURPOSE  : Perform addition y(n) = x1(n) + x2(n)
...
function [yn, yorigin] = add(x1n, x1origin, x2n, x2origin)
    ...
    ny = nmin:nmax;
    yorigin = 1 - nmin;
    y1 = zeros(1, length(ny)); y2 = zeros(1, length(ny));
    y1(n1 - nmin + 1) = x1n;           // zero-pad x1(n)
    y2(n2 - nmin + 1) = x2n;           // zero-pad x2(n)
    yn = y1 + y2;                       // element-wise addition
...
// ------------------------------------------------------------
// Example: x1(n) = {0^, 1, 3, -2}, x1origin = 1
//          x2(n) = {-1, 1^, 2,  3}, x2origin = 2
// Result : yn = [-1, 1, 3, 6, -2],  yorigin = 2
// ------------------------------------------------------------
